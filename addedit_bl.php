...
<div class="wrapper">
  ...
  <div class="page-title-box">
	<div class="container-fluid">
		<div class="row">
			<div class="col-sm-12">
				<h4 class="page-title">Gestion des livraisons client</h4>
  ...
  <script>
		function Supprimer(id,idd)
	  {
			...
			{
				document.location.href="page_js/supprimerdet_bl.php?IDD="+idd+"&ID="+id ;
			}			    
	  }	
  </script>
 <?php
	...
	if(isset($_GET['ID'])){
		...
		$reqbe="select * from delta_bl where exercice=".$exercice;
...
if(isset($_POST['enregistrer_mail'])){	
	...
	if($id==0){
			...
			$reqMax="select max(id) as maxID from delta_bl";
			...
			$sql="INSERT INTO `delta_bl`(`id`, `numero`, `date`, `client`, `observation`, `idutilisateur`, `dateheure`, `exercice`, `magasin`, `codsoc`, `num_exoneration`, `date_debut`, `date_fin`, `nature_client`) VALUES 
			('".$id."','".$reference."','".$date."','".$client."','".$remarque."','".$idutilisateur."','".$dateheure."','".$exercice."','".$magasin."','".$codsoc."','".$num_exoneration."','".$date_debut."','".$date_fin."','".$nature."')";
			...
			$sql1="INSERT INTO `delta_log`(`dateheure`, `idutilisateur`, `document`, `action`, `iddocument`) VALUES ('".$dateheure."','".$iduser."','22','1','".$id."')";
			$req=mysql_query($sql1);

			
			//Insertion détails
			$sql="INSERT INTO `delta_det_bl`(`idbl`, `produit`, `fodec`, `taux_fodec`, `total_fodec`, `px_ht`, `tva`, `val_tva`, `px_ttc`, `quantite`, `quantite_colis`, `quantite_par_colis`, `px_ht_total`, `taux_remise`, `valeur_remise`, `px_total`, `px_ht_total_remise`, `date_livraison`, `colisage`) VALUES
		...
		if($idd==0){
			...
			$ancien_qte		=	0;
			$req="select * from delta_det_bl where id=".$idd;
			$query=mysql_query($req);
			while($enreg=mysql_fetch_array($query)){
				$ancien_qte		=	$enreg['quantite'];
			}			
			//Mise à jours table détail
			$sql="update delta_det_bl set px_ht='".$px_ht."',tva='".$tva."',val_tva='".$val_tva."',px_ttc='".$px_ttc."',quantite='".$quantite."'
			...
			$sql1="INSERT INTO `delta_log`(`dateheure`, `idutilisateur`, `document`, `action`, `iddocument`) VALUES ('".$dateheure."','".$iduser."','22','2','".$id."')";
			$req=mysql_query($sql1);			
		}		
	}
	...
	$total = 0;
	$req="select sum(px_total) as total from delta_det_bl where idbl=".$id;
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query)){
		$total = $enreg['total'];
	}
			
	$sql="update delta_bl set montant=".$total." where id=".$id;
	$requete=mysql_query($sql);				
	

		echo '<SCRIPT LANGUAGE="JavaScript">document.location.href="addedit_bl.php?ID='.$id.'&suc=1" </SCRIPT>';
	...
	$date				=	date("Y-m-d");
	$req="select  * from delta_bl where id=".$id;
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query)){
		$client				=	$enreg['client'];
			...
			<div class="row">
				<div class="col-lg-12">
					<div class="card m-b-20">
						<div class="card-body">
							<a href="gest_bl.php" class="btn btn-primary waves-effect waves-light">Retour</a> 
							...
							<form action="" method="POST"   enctype="multipart/form-data">    
								<div class="form-group row">
									...
									<b>Liste des magasins (*)</b>
										<select class="form-control select2" name="magasin" id="magasin" required>
											<option value="0"> Sélectionner une magasin </option>
											<?php
											if($magasin=="0"){
									...
									<b>Liste des produits (*)</b>
										<select class="form-control select2" name="produit" id="produit" required>
											...
											if($produit==0){
												...
												select * from delta_det_bl where delta_det_bl.produit=delta_produits.id and idbl=".$id.") order by reference";
											} else{
...
<?php
	$total = 0;
	$req="select montant from delta_bl where id=".$id;
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query)){
		$total = $enreg['montant'];
	}
							...
							<?php } ?>
								<br>
                                    <table class="table mb-0">
                                        ...
                                        <tbody>
<?php
	$req="select * from delta_det_bl where idbl=".$id;
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query)){
		$produit = "";
...
?>
                                        <tr>
											...
											<td>
												<a href="addedit_bl.php?ID=<?php echo $id; ?>&IDD=<?php echo $enreg['id']; ?>" class="btn btn-warning waves-effect waves-light">
...
<script>
	...
	$("#produit").on("change", function(){
		...
		if(magasin==0){
			alert("Sélectionner une magasin SVP");
			$('#produit').append('<option value="0" selected="selected">Sélectionner un produit</option>');		
			return false;
		}			
		var produit = $("#produit").val();
		if(produit>0){
			...
			xmlhttp_selectPrd.open("POST","page_json/json_detail_produit_bl.php",true);
			xmlhttp_selectPrd.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			xmlhttp_selectPrd.send("produit="+produit+"&client="+client+"&magasin="+magasin);	
			$("#DetailProduit").show();			
		} else{
			$("#DetailProduit").hide();
			$("#spanProduit").show();
