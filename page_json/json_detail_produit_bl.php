<?php
	...
	$stock_magasin			=	0;
	$reqmag="select * from delta_stock_magasin where produit=".$produit." and magasin=".$magasin;
	$querymag=mysql_query($reqmag);
	while($enregmag=mysql_fetch_array($querymag)){
		$stock_magasin		=	$enregmag['stock'];
	}
	...
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query)){
		$taux_fodec			=	$enreg['fodec'];
	}
	
	if($fodec==1){
		$total_fodec 		=  round(($px_ht/100)*$taux_fodec, 3);
		$px_ht				=  $px_ht + $total_fodec;
	}	
...
?>
	<div class="col-md-12 row" style="margin-top:05px; margin-bottom:20px">
		<div class="col-md-6">
			<b style="color:green"> Stock Dispnible : <?php echo $stock; ?></b> | <b style="color:blue"> Stock Magasin : <?php echo $stock_magasin; ?></b>
		</div>
	</div>
