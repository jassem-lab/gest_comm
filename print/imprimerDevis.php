...
while($enreg=mysql_fetch_array($query))
{
		...
		$reqfm="select * from delta_det_devis where idd=".$id ;
		$queryfm=mysql_query($reqfm);
		while($enregfm=mysql_fetch_array($queryfm)){
			$produit		        =	$enregfm["produit"] ;
			$tva		    		=	$enregfm["tva"] ;
			$prix_vente_ht			=	$enregfm["prix_vente_ht"] ;
			$prix_vente_ttc			=	$enregfm["prix_vente_ttc"] ;
			$taux_remise			=	$enregfm["taux_remise"] ;
			$prix_remise			=	$enregfm["prix_remise"] ;
			$quantite			    =	$enregfm["quantite"] ;
			$prix_total			    =	$enregfm["prix_total"] ;
		}
        $reqA = "select * from delta_clients where id=".$client; 
        $queryA = mysql_query($reqA); 
        while($enregA = mysql_fetch_array($queryA)){
            $raison_social  = $enregA["raison_social"];
            $adresse        = $enregA["adresse"] ; 
            $tel            = $enregA["tel"] ; 
        }
}
...
$total_ttc=0;
$total=0;$tot_qte=0;
$pdf->SetWidths(array(15,30,30,30,30,30,15,15));
srand(microtime()*1000000);
$pdf->Ln();$pdf->Ln();
$pdf->RowHead(array(utf8_decode("produit"),utf8_decode("TVA"),utf8_decode("Prix de vente HT"),utf8_decode("Prix de vente TTC"),utf8_decode("Taux de remise"),utf8_decode("Prix après Remise"),utf8_decode("Quantité"), utf8_decode("Prix Total")));
	$ref="";
	...
	{   
		...
		// $pdf->Row(utf8_decode($numero),utf8_decode($produit),array(utf8_decode($tva),utf8_decode($prix_vente_ht),utf8_decode($prix_vente_ttc),utf8_decode($taux_remise),number_format($prix_remise,"3","."," "),($quantite),number_format($prix_total,"3","."," ") ,));
		
        $pdf->SetFont('Arial','', 12); // Font Name, Font Style (eg. 'B' for Bold), Font Size
		$pdf->Row(array(utf8_decode($produit),utf8_decode($tva.'%'),utf8_decode($prix_vente_ht),utf8_decode($prix_vente_ttc),utf8_decode($taux_remise),utf8_decode($prix_remise),utf8_decode($quantite),utf8_decode($prix_total)
		   ));		
